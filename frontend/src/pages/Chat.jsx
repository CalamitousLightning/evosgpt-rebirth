    ...
    <div style={S.wrap}>
      ...
      <div style={S.body}>
        {loadingHist ? (
          ...
          <div style={S.emptyState}>
            ...
            <div style={{ fontWeight: 800, fontSize: 16, color: "#f1f5f9", marginBottom: 6 }}>Hey {user?.full_name?.split(" ")[0] || user?.username}!</div>
            <div style={{ color: "#64748b", fontSize: 14 }}>Ask me anything — code, letters, ideas, business plans. I'll remember as we chat.</div>
          </div>
        ) : (
          messages.map((m, i) => (
      ...
      {limitHit && (
        <div style={S.limitBox} className="slide-up">
          ...
          <p style={{ fontSize: 13, color: "#94a3b8", margin: "4px 0 10px" }}>Upgrade to Pro for unlimited chats — or come back tomorrow.</p>
          <div style={{ display: "flex", gap: 8 }}>
      ...
      <div style={S.inputBar}>
        ...
        />
        <button style={{ ...S.sendBtn, opacity: (!input.trim() || sending || limitHit) ? 0.4 : 1 }} onClick={send} disabled={!input.trim() || sending || limitHit}>
          ➤
        </button>
      </div>
    </div>
...
const S = {
  ...
  body:   { flex: 1, overflowY: "auto", padding: "20px 16px 10px", display: "flex", flexDirection: "column", gap: 12, maxWidth: 760, margin: "0 auto", width: "100%" },
  emptyState: { textAlign: "center", margin: "auto", maxWidth: 320 },
  msgRow: { display: "flex" },
  bubble: { maxWidth: "82%", padding: "12px 16px", borderRadius: 16, fontSize: 14.5, lineHeight: 1.6, wordBreak: "break-word" },
  userBubble: { background: "linear-gradient(135deg,#38bdf8,#0ea5e9)", color: "#000", fontWeight: 600, borderBottomRightRadius: 4 },
